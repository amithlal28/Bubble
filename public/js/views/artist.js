/* ═══════════════════════════════════════════════════════════════════
   STASH DESKTOP — Artist Detail View
   Shows artist tracks with Play button
   ═══════════════════════════════════════════════════════════════════ */
  ...
  container.innerHTML = `
    <div style="display:flex;gap:var(--space-2xl);align-items:flex-start;flex-wrap:wrap;padding-bottom:var(--space-2xl)">
      ...
      <div style="flex:1;min-width:280px">
        <div style="font-size:var(--font-xs);font-weight:700;letter-spacing:1px;text-transform:uppercase;color:var(--text-tertiary);margin-bottom:8px">Artist</div>
        <h1 style="font-size:var(--font-2xl);font-weight:800;margin-bottom:var(--space-lg)">${escapeHtml(artistName)}</h1>
        ...
        <div style="display:flex;align-items:center;gap:var(--space-md);margin-bottom:var(--space-xl);color:var(--text-tertiary);font-size:var(--font-sm)">
          ...
          <span>•</span>
          <span>${albumSet.size} albums</span>
          <span>•</span>
        ...
        <div style="display:flex;gap:var(--space-md);flex-wrap:wrap">
          <button class="btn btn-primary" style="padding:10px 28px;font-size:var(--font-base)" onclick="if(window.__artistTracks){BubblePlayer.setQueue(window.__artistTracks,0)}">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21"/></svg> Play All
          </button>
          <button class="btn btn-secondary" style="padding:10px 20px" onclick="if(window.__artistTracks){BubblePlayer.setQueue(window.__artistTracks,0,true)}">
          ...
          <button class="btn btn-secondary" style="padding:10px 20px" id="btn-save-artist" onclick="BubbleApp.saveArtistToLibrary('${escapeHtml(artistName)}','${escapeHtml(artworkUrl)}')">
    ...
    ${tracks.length > 0 ? `
      <div class="track-list">
        ...
        ${tracks.map((t, i) => renderArtistTrackRow(t, i)).join('')}
      </div>
    ` : `
      <div class="empty-state">
        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
        <div class="empty-state-title">No tracks found</div>
        <div class="empty-state-text">No tracks were found for this artist. Try searching directly.</div>
      </div>
    `}
  `;
});
